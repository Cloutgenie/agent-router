...
export function StageStepper({ status }: { status: TaskStatus }) {
      ...
      {STAGES.map((stage, index) => {
        ...
        return (
          <div key={stage.key} className="flex items-center gap-1 sm:gap-2">
            ...
            >
              {isActive && <span className="h-1.5 w-1.5 animate-pulse-dot rounded-full bg-accent-strong" />}
              {isDone && <span>✓</span>}
              {stage.label}
            </div>
            {index < STAGES.length - 1 && <div className={`h-px w-3 sm:w-6 ${isDone ? "bg-good/40" : "bg-border"}`} />}
          </div>
        );
      })}
    </div>
